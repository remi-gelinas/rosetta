...
mkEmacsPackage "default" (
  ...
  let
    ...
    # Package requires
    requiresPackages = lib.attrsets.mapAttrsToList (_: pkg: pkg.finalPackage) allConfigPackages;

    requires =
      with lib;
      pipe allConfigPackages [
        (attrsets.mapAttrsToList (_: pkg: "(require '${pkg.name})"))
        (builtins.concatStringsSep "\n")
      ];

    # Make binaries required by packages available
    allRequiredBinaryPackages =
      with lib;
      pipe allConfigPackages [
