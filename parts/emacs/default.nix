...
in
{
  imports = import ./configs { inherit localFlake mkEmacsPackage; };
  ...
  perSystem =
    ...
    let
      ...
      configPackageType = types.submodule (
        { config, ... }:
        {
          options = {
          ...
          config.finalPackage =
            ...
            emacsPackages.trivialBuild {
              ...
              packageRequires =
                ...
                in
                if type == "lambda" then
                  config.requiresPackages emacsPackages
                # then type is list - maybe explicitly throw here? Should be handled by the option type check
                else
                  config.requiresPackages;
            };
        }
      );
    in
    {
      options.emacs = {
        configPackages = mkOption { type = types.attrsOf configPackageType; };
